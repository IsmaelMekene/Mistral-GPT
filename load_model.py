...
class LlamaCPPInvocationLayer(PromptModelInvocationLayer):
    def __init__(
        ...
        **kwargs,
        ...
        For more details on kwargs, see: https://abetlen.github.io/llama-cpp-python/#llama_cpp.llama.Llama.__init__  # noqa: E501
        """
        if model_name_or_path is None or len(model_name_or_path) == 0:
            raise ValueError("model_name_or_path cannot be None or an empty string")  # noqa: E501
    ...
    def _ensure_token_limit(
        ...
        """
        Ensure that the length of the prompt and answer is within
        the maximum token length of the PromptModel.

        :param prompt: Prompt text to be sent to the generative model.
        :return: A prompt that fits within the token length limit.
        """
        if not isinstance(prompt, str):
            raise ValueError(f"Prompt must be of type str but got {type(prompt)}")  # noqa: E501
        ...
        if len(tokenized_prompt) + self.max_length > context_length:
            logger.warning(
                "The prompt has been truncated from %s tokens to %s ,tokens so that the prompt length and "  # noqa: E501
                "answer length (%s tokens) fit within the max token limit (%s tokens). "  # noqa: E501
            ...
            return bytes.decode(
                self.model.detokenize(
                    tokenized_prompt[: max(0, context_length - self.max_length)]  # noqa: E501
                ),
                "utf-8",
            )
    ...
    def invoke(self, *args, **kwargs):
        """
        Takes a prompt and returns a list of
        generated text using the underlying model.
        ...
        if kwargs and "prompt" in kwargs:
            ...
            # For more details, refer to the call documentation for Llama CPP:
            # https://abetlen.github.io/llama-cpp-python/#llama_cpp.llama.Llama.__call__
            model_input_kwargs = {
                key: kwargs[key]
                for key in [
                    "suffix",
    ...
    def supports(cls, model_name_or_path: str, **kwargs) -> bool:
        ...
        the underlying model which might be used to determine
        if the model is supported.
        :return: True if this invocation layer supports the model,
        False otherwise.
        """
