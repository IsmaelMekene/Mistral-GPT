...
from haystack.nodes import EmbeddingRetriever, PromptModel, PromptNode, PromptTemplate, AnswerParser
...
# Use FastAPI's dependency injection for creating document store and query pipeline
...
def get_query_pipeline():
    ...
    query_pipeline.add_node(component=PromptNode(
        model_name_or_path="model/mistral-7b-instruct-v0.1.Q4_K_S.gguf",
        max_length=1000,
        default_prompt_template=PromptTemplate(prompt="Your prompt here", output_parser=AnswerParser())
...
async def get_answer(question: str = Form(...), document_store = Depends(get_document_store), query_pipeline = Depends(get_query_pipeline)):
    json_response = query_pipeline.run(query=question, params={"Retriever": {"top_k": 5}})
    ...
    relevant_documents = "\n".join([f"Document {i + 1} Content:\n{content}" for i, content in enumerate(document_info)])
